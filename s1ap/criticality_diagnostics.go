...
func (d CriticalityDiagnostics) MarshalPER(w *per.Writer, enc per.Encoding) error {
	...
	if len(d.IEsCriticalityDiagnostics) > 0 {
		if len(d.IEsCriticalityDiagnostics) > maxnoofErrors {
			return fmt.Errorf("s1ap: %d criticality-diagnostics items exceed maxnoofErrors", len(d.IEsCriticalityDiagnostics))
		}
...
func (d *CriticalityDiagnostics) UnmarshalPER(r *per.Reader, enc per.Encoding) error {
	...
	for i := range bits {
		b, err := r.ReadBit()
		if err != nil {
			return fmt.Errorf("s1ap: criticality diagnostics preamble: %w", err)
		}
	...
	*d = CriticalityDiagnostics{}
	...
	if bits[1] {
		...
		if err != nil {
			return fmt.Errorf("s1ap: criticality diagnostics procedureCode: %w", err)
		}
	...
	if bits[2] {
		...
		if err != nil {
			return fmt.Errorf("s1ap: criticality diagnostics triggeringMessage: %w", err)
		}
	...
	if bits[3] {
		...
		if err != nil {
			return fmt.Errorf("s1ap: criticality diagnostics procedureCriticality: %w", err)
		}
