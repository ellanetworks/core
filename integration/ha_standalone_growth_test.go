...
func TestIntegrationHAStandaloneGrowsIntoCluster(t *testing.T) {
	...
	for _, m := range members {
		if m.Suffrage != "voter" {
			t.Errorf("node %d joined as %q, want voter", m.NodeID, m.Suffrage)
		}
	}
	...
	if err != nil {
		t.Fatalf("no new leader after draining node %d: %v", leaderStatus.Cluster.NodeID, err)
	}
	...
	if newLeaderStatus.Cluster.NodeID == leaderStatus.Cluster.NodeID {
		t.Fatalf("leadership never moved off node %d", leaderStatus.Cluster.NodeID)
	}
}
