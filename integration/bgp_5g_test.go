...
func TestIntegration5GBGP(t *testing.T) {
	...
	t.Run("RouteAdvertisement", func(t *testing.T) {
		sc, _ := scenarios.Get("gnb/session_hold")
		...
		argv := []string{
			"core-tester", "run", "gnb/session_hold",
		...
		t.Cleanup(func() {
			_, _ = dc.Exec(ctx, testerContainer, []string{"pkill", "-f", "gnb/session_hold"}, true, 5*time.Second, nil)
		})
