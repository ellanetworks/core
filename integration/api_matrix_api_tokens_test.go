...
func runAPITokensMatrix(ctx context.Context, t *testing.T, c *client.Client) {
	...
	// Self-scoped tokens (/users/me/api-tokens) operate on the caller's own
	// tokens; only the uniquely named token created here is deleted, so the
	// bootstrap token the test client authenticates with is never touched.
	t.Run("self_scoped", func(t *testing.T) {
		const tokenName = "apimat-my-token"

		listMine := func() *client.ListAPITokensResponse {
