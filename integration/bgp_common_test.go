...
// runBGPSuite drives the full BGP integration suite against a held UE session.
// The BGP control-plane assertions (gobgp neighbor up, the external peer's RIB
// actually receiving Ella Core's advertised UE route, inbound learning, prefix
// filtering, safety filter, withdrawal, peer deletion, NAT-toggle) are
// RAT-agnostic and shared. Only the session-hold scenario differs: 5G drives
// gnb/session_hold (a PDU session), 4G drives s1enb/session_hold (an EPS
// bearer). Both pin a UE IP into the advertised pool the same way.
func runBGPSuite(t *testing.T, sessionHoldScenario string) {
	t.Helper()
	...
	t.Run("RouteAdvertisement", func(t *testing.T) {
		sc, _ := scenarios.Get(sessionHoldScenario)
		...
		argv := []string{
			"core-tester", "run", sessionHoldScenario,
		...
		t.Cleanup(func() {
			_, _ = dc.Exec(ctx, testerContainer, []string{"pkill", "-f", sessionHoldScenario}, true, 5*time.Second, nil)
		})
