...
// flow content — the same depth as the 5G test. The UPF rule engine and flow
// accounting are RAT-agnostic; the 4G probe (s1enb/connectivity_expect_*) drives
// the same protocols, port (34242), and payloads as the 5G one.
//
// IPv4 only for now: the s1enb connectivity_expect scenarios have no IPv6 variant
// yet (the 5G test covers IPv6 via gnb/connectivity_expect_*_ipv6).
