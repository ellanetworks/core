...
func runNATChecksumSuite(t *testing.T, scenarioName string) {
	...
	sc, ok := scenarios.Get(scenarioName)
	Assert(t, ok, fmt.Sprintf("scenario %q not registered", scenarioName))
	...
	env.RunScenario(ctx, t, scenarioName, tr, "--probe-payload-bytes", "16,500,800,1300")
