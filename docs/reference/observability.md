...
| app_upf_bpf_map_pressure_ratio | Fill ratio of a data plane BPF map, between 0 and 1, labeled by `map`. Only reported for a map at or above 75% of its capacity, so no series exists in normal operation. | Gauge |
