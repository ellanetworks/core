...
| app_upf_bpf_map_memory_bytes | Kernel memory locked by a data plane BPF map, labeled by `map`. This memory is held by the kernel and is not part of `process_resident_memory_bytes`. | Gauge |
