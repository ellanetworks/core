...
| app_upf_datapath_drop_total | Packets the data plane did not forward, with labels for direction (uplink, downlink) and reason. Together with app_upf_datapath_forward_total this accounts for every frame exactly once. Reasons prefixed `internal_` are datapath failures and should stay at zero; `unspecified` means a drop site recorded no cause and is a bug. | Counter |
| app_upf_datapath_fib_lookup_total | FIB lookup outcomes in the data plane, with labels for interface (n3, n6) and result matching kernel return codes (success, no_neigh, blackhole, unreachable, prohibit, no_src_addr, frag_needed, not_fwded, fwd_disabled, unsupp_lwt), plus error_ipv4 and error_ipv6 for a lookup the kernel rejected. The outcomes that drop the packet are also counted in app_upf_datapath_drop_total under their `fib_` reason; this metric is the full distribution, including the outcomes that forward. | Counter |
