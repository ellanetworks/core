...
When the new binary carries schema changes, the cluster keeps running on the old schema until every member of the Raft configuration reports support live via its cluster-internal status endpoint; only then does the migration commit through Raft. Non-voters count: they apply the same committed entries as voters, so an old binary on a non-voter holds the migration back exactly as a voter does.
