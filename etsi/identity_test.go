...
func TestNewGutiFromBytes_Invalid(t *testing.T) {
	...
	testcases := []Testcase{
		{"tooshort", []byte{0xf2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas/fgs: 5G-GUTI is 10 octets, want 11"},
		{"toolong", []byte{0xf2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas/fgs: 5G-GUTI is 12 octets, want 11"},
		{"invalidmccdigit1", []byte{0xf2, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas: PLMN digit at octet 0: non-decimal digit"},
		{"invalidmccdigit2", []byte{0xf2, 0xa0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas: PLMN digit at octet 1: non-decimal digit"},
		{"invalidmccdigit3", []byte{0xf2, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas: PLMN digit at octet 2: non-decimal digit"},
		{"invalidmncdigit1", []byte{0xf2, 0x00, 0xa0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas: PLMN digit at octet 5: non-decimal digit"},
		{"invalidmncdigit2", []byte{0xf2, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas: PLMN digit at octet 3: non-decimal digit"},
		{"invalidmncdigit3", []byte{0xf2, 0x00, 0x00, 0xa0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, "nas: PLMN digit at octet 4: non-decimal digit"},
		{"invalidtmsi", []byte{0xf2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff}, "invalid TMSI"},
	}
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestNewGUTIFromBytes_Valid(t *testing.T) {
	...
	testcases := []Testcase{
		{"00101000001deadbeef", []byte{0xf2, 0x00, 0xf1, 0x10, 0x00, 0x00, 0x01, 0xde, 0xad, 0xbe, 0xef}},
		{"001001cafe42deadbeef", []byte{0xf2, 0x00, 0x11, 0x00, 0xca, 0xfe, 0x42, 0xde, 0xad, 0xbe, 0xef}},
		{"99999cafe42deadbeef", []byte{0xf2, 0x99, 0xf9, 0x99, 0xca, 0xfe, 0x42, 0xde, 0xad, 0xbe, 0xef}},
		{"30242450514deadbeef", []byte{0xf2, 0x03, 0xf2, 0x24, 0x45, 0x05, 0x14, 0xde, 0xad, 0xbe, 0xef}},
	}
	for _, tc := range testcases {
		t.Run(tc.expected, func(t *testing.T) {
			guti, err := etsi.NewGUTI5GFromBytes(tc.buf)
