...
func GutiToNas(guti string) nasType.GUTI5G {
	...
	if mcc1Tmp, err := strconv.Atoi(string(guti[0])); err != nil {
		logger.UtilLog.Warnf("atoi mcc1 error: %+v", err)
	} else {
		mcc1 = mcc1Tmp
	}
	if mcc2Tmp, err := strconv.Atoi(string(guti[1])); err != nil {
		logger.UtilLog.Warnf("atoi mcc2 error: %+v", err)
	} else {
		mcc2 = mcc2Tmp
	}
	if mcc3Tmp, err := strconv.Atoi(string(guti[2])); err != nil {
		logger.UtilLog.Warnf("atoi mcc3 error: %+v", err)
	} else {
		mcc3 = mcc3Tmp
	}
	if mnc1Tmp, err := strconv.Atoi(string(guti[3])); err != nil {
		logger.UtilLog.Warnf("atoi mnc1 error: %+v", err)
	} else {
		mnc1 = mnc1Tmp
	}
	if mnc2Tmp, err := strconv.Atoi(string(guti[4])); err != nil {
		logger.UtilLog.Warnf("atoi mnc2 error: %+v", err)
	} else {
		mnc2 = mnc2Tmp
	}
	mnc3 = 0x0f
	...
	if len(guti) == 20 {
		if mnc3Tmp, err := strconv.Atoi(string(guti[5])); err != nil {
			logger.UtilLog.Warnf("atoi guti error: %+v", err)
		} else {
			mnc3 = mnc3Tmp
		}
		amfId = guti[6:12]
	...
	if tmsiBytes, err := hex.DecodeString(tmsi); err != nil {
		logger.UtilLog.Warnf("decode TMSI failed: %+v", err)
	} else {
