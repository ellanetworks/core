...
func HandleERABSetupResponse(ctx context.Context, m *mme.MME, radio *mme.Radio, value []byte) {
	msg, err := s1ap.ParseERABSetupResponse(value)
	if err != nil {
		logger.From(ctx, logger.MmeLog).Warn("failed to decode E-RAB Setup Response", zap.Error(err))
		return
	}
