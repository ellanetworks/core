...
func Route(ctx context.Context, m *mme.MME, radio *mme.Radio, pdu any) {
	...
	case *s1ap.InitiatingMessage:
		...
		default:
			logger.From(ctx, radio.Log()).Warn("unsupported initiating procedure", zap.Int64("procedure_code", int64(p.ProcedureCode)))
	...
	case *s1ap.SuccessfulOutcome:
		...
		default:
			logger.From(ctx, radio.Log()).Warn("ignoring unsupported procedure", zap.String("kind", "successful-outcome"), zap.Int64("procedure_code", int64(p.ProcedureCode)))
		}
	case *s1ap.UnsuccessfulOutcome:
		...
		default:
			logger.From(ctx, radio.Log()).Warn("ignoring unsupported procedure", zap.String("kind", "unsuccessful-outcome"), zap.Int64("procedure_code", int64(p.ProcedureCode)))
		}
	default:
