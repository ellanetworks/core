...
// decoded reports whether a parse produced a message the MME may act on, and
// logs the optional elements it had to drop.
//
// TS 24.301 §7.7.1 requires the network to treat a syntactically incorrect
...
func decoded(ctx context.Context, message string, err error) bool {
	...
	logger.From(ctx, logger.MmeLog).Warn("ignoring syntactically incorrect optional IEs (TS 24.301 §7.7.1)",
