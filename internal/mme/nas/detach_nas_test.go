...
func TestForgedMessageIgnoredForSecuredUE(t *testing.T) {
	...
	HandleNAS(m, context.Background(), ue, forged)

	if cc.count() != 0 {
		t.Fatalf("forged DETACH against a secured UE was acted on: %d downlink(s) sent", cc.count())
	}

	if _, ok := m.LookupUe(ue.S1.MMEUES1APID); !ok || ue.EMMState() != mme.EMMRegistered || !ue.SecuredForTest() {
...
func TestDetachSwitchOff(t *testing.T) {
	...
	HandleNAS(m, context.Background(), ue, detachRequest(t, ue, true))
...
func TestDetachSwitchOffUnsecuredAccepted(t *testing.T) {
	...
	ue.S1.SetSecureExchangeEstablishedForTest(false)
...
func TestDetachNotSwitchOff(t *testing.T) {
	...
	HandleNAS(m, context.Background(), ue, detachRequest(t, ue, false))
