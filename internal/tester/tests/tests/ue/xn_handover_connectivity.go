...
import (
	...
	"github.com/ellanetworks/core/internal/tester/tests/tests/utils/validate"
...
func (t XnHandoverConnectivity) Run(ctx context.Context, env engine.Env) error {
	...
	cmd := exec.CommandContext(ctx, "ping", "-I", preTunName, env.Config.PingDestination, "-c", "3", "-W", "1") // #nosec G204 -- ping is fixed; preTunName is internally derived; PingDestination is test config
	...
	// Ping AFTER handover
	cmd = exec.CommandContext(ctx, "ping", "-I", postTunName, env.Config.PingDestination, "-c", "3", "-W", "1") // #nosec G204 -- ping is fixed; postTunName is internally derived; PingDestination is test config
