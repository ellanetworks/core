...
func runClusterTraffic(ctx context.Context, env scenarios.Env, p *clusterTrafficParams) error {
	...
	if g.N2Address == "" || g.N3Address == "" {
		return fmt.Errorf("multi/cluster_traffic_5g requires a --gnb declaration with n2 and n3")
	}
	...
	logger.Logger.Info("multi/cluster_traffic_5g: gNB up, driving UEs",
	...
	logger.Logger.Info("multi/cluster_traffic_5g: all UEs pinged successfully",
		zap.String("gnbId", p.GnbID),
		zap.Int("ueCount", p.UECount),
	)
