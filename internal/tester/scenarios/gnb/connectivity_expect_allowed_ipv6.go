...
func init() {
	scenarios.Register(scenarios.Scenario{
		Name: "gnb/connectivity_expect_allowed_ipv6",
...
func fixtureConnectivityExpectAllowedIPv6(env scenarios.Env) scenarios.FixtureSpec {
...
func runConnectivityExpectAllowedIPv6(ctx context.Context, env scenarios.Env, params *connectivityProbeParams) error {
	...
	for i := range numConnectivityParallel {
		func() {
			eg.Go(func() error {
				...
				return runConnectivityExpectAllowedIPv6Test(
					ctx,
					ranUENGAPID,
					gNodeB,
					subs[i],
	...
	if err := eg.Wait(); err != nil {
		return fmt.Errorf("error during connectivity_expect_allowed_ipv6 test: %v", err)
	}

	return nil
}

func runConnectivityExpectAllowedIPv6Test(
	ctx context.Context,
	ranUENGAPID int64,
	gNodeB *gnb.GnodeB,
	sub subscriber,
