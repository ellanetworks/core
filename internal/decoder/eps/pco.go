...
// ExtendedPCO renders the (extended) protocol configuration options element
// (TS 24.008 §10.5.6.3). The element predates 5GS and both stacks carry it, so
// one rendering serves them.
func ExtendedPCO(opts *nas.ProtocolConfigurationOptions) *ExtendedProtocolConfigurationOptions {
	if opts == nil {
		return nil
	}
