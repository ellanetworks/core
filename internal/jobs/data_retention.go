...
func StartDataRetentionWorker(database *db.Database) {
	go func() {
		...
		for {
			...
			if err := enforceSubscriberDataRetention(database); err != nil {
...
func enforceSubscriberDataRetention(database *db.Database) error {
	ctx := context.Background()

	days, err := database.GetRetentionPolicy(ctx, db.CategorySubscriberLogs)
	if err != nil {
		return err
	}
