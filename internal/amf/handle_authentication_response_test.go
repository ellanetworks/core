...
func TestHandleAuthenticationResponse_NilAuthenticationResponseParameter(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = handleAuthenticationResponse(context.TODO(), New(nil, nil, nil), ue,
...
func TestHandleAuthenticationResponse_PreconditionErrors(t *testing.T) {
	...
	testcases := []TestCase{
		{
			...
			fmt.Errorf("state mismatch: receive Authentication Response message in state %s", Deregistered),
		},
		{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			err := handleAuthenticationResponse(context.TODO(), New(nil, nil, nil), tc.ue, &nasMessage.AuthenticationResponse{AuthenticationResponseParameter: &nasType.AuthenticationResponseParameter{}})
...
func TestHandleAuthenticationResponse_TimerT3560Stopped(t *testing.T) {
	...
	conn.T3560 = NewTimer(10*time.Minute, 5, func(e int32) {}, func() {})

	_ = handleAuthenticationResponse(t.Context(), New(nil, nil, nil), ue, &nasMessage.AuthenticationResponse{AuthenticationResponseParameter: &nasType.AuthenticationResponseParameter{}})
...
func TestHandleAuthenticationResponse_hResStartMismatch(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = handleAuthenticationResponse(t.Context(), New(nil, nil, nil), ue, &nasMessage.AuthenticationResponse{AuthenticationResponseParameter: &nasType.AuthenticationResponseParameter{}})
			if err != nil {
				t.Fatalf("expected no error, got: %v", err)
			}
...
func TestHandleAuthenticationResponse_Auth5gAKA_Failure(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			amfInstance := New(&FakeDBInstance{
