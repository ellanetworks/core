...
func CreateN1N2MessageTransfer(ueContextId string, n1n2MessageTransferRequest models.N1N2MessageTransferRequest, reqUri string) (*models.N1N2MessageTransferRspData, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not handle N1N2MessageTransfer Request: %v", err)
	...
	return nil, fmt.Errorf("could not handle N1N2MessageTransfer Request")
}

// There are 2 possible return value for this function:
//   - n1n2MessageTransferRspData: if AMF handle N1N2MessageTransfer Request successfully.
//   - error: if AMF reject the request due to application error, e.g. UE context not found. or due to a procedure error, e.g. UE has an ongoing procedure.
//
...
func N1N2MessageTransferProcedure(ueContextID string, reqUri string,
	...
	if anType == models.AccessType__3_GPP_ACCESS {
		if requestData.SkipInd && n2Info == nil {
			...
			if err != nil {
				return n1n2MessageTransferRspData, fmt.Errorf("build Paging failed : %s", err.Error())
			}
		...
		if ue.CmConnect(models.AccessType__3_GPP_ACCESS) {
			if n2Info == nil {
				...
				if err != nil {
					return n1n2MessageTransferRspData, fmt.Errorf("build Notification failed : %s", err.Error())
				}
