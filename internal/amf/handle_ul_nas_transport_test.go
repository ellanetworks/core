...
func TestHandleULNASTransport_WrongState_Error(t *testing.T) {
	testcases := []StateType{Deregistered, Authentication, SecurityMode, ContextSetup}
	for _, tc := range testcases {
		t.Run(string(tc), func(t *testing.T) {
			...
			err = handleULNASTransport(t.Context(), New(nil, nil, nil), ue, msg, true)
			if err == nil {
...
func TestHandleULNASTransport_MacFailed_Error(t *testing.T) {
	...
	err = handleULNASTransport(t.Context(), New(nil, nil, nil), ue, msg, false)
...
func TestTransport5GSMMessage_NilPduSessionID_Error(t *testing.T) {
	...
	err = transport5GSMMessage(t.Context(), New(nil, nil, nil), ue, msg)
	if err == nil || err.Error() != expected {
		t.Fatalf("expected error: %s, got: %v", expected, err)
	}
}
...
func TestForward5GSMMessageToSMF_UpdateError_ReturnsError(t *testing.T) {
	...
	err = forward5GSMMessageToSMF(t.Context(), amfInstance, ue, 1, "ref-1", []byte{0x01})
	if err == nil {
		t.Fatal("expected an error, got nil")
	}
