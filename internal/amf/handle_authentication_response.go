...
// TS 24.501 5.4.1
func handleAuthenticationResponse(ctx context.Context, amfInstance *AMF, ue *UeContext, msg *nasMessage.AuthenticationResponse) error {
	if state := ue.GetState(); state != Authentication {
...
func failAuthentication(ctx context.Context, ue *UeContext, ranUe *RanUe, conn *ActiveNasConnection) error {
	if conn.IdentityTypeUsedForRegistration == nasMessage.MobileIdentity5GSType5gGuti {
		if err := SendIdentityRequest(ctx, ranUe, nasMessage.MobileIdentity5GSTypeSuci); err != nil {
	...
	if err := SendAuthenticationReject(ctx, ranUe); err != nil {
