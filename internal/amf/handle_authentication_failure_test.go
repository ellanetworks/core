...
func TestHandleAuthenticationFailure_WrongState_Error(t *testing.T) {
	testcases := []StateType{Deregistered, SecurityMode, ContextSetup, Registered}
	for _, tc := range testcases {
		t.Run(fmt.Sprintf("State-%s", tc), func(t *testing.T) {
			...
			err = handleAuthenticationFailure(t.Context(), New(nil, nil, nil), ue, msg)
...
func TestHandleAuthenticationFailure_T3560Stopped(t *testing.T) {
	...
	_ = handleAuthenticationFailure(t.Context(), New(nil, nil, nil), ue, msg)
