...
func HandleUEContextReleaseComplete(ctx context.Context, amf *amfContext.AMF, ran *amfContext.Radio, msg *ngapType.UEContextReleaseComplete) {
	...
	if amfUe.GetState() == amfContext.Registered {
		ranUe.Log.Debug("Release UE Context in GMM-Registered", logger.SUPI(amfUe.Supi.String()))
	...
	case amfContext.UeContextN2NormalRelease:
		ran.Log.Info("Release UE Context: N2 Connection Release", logger.SUPI(amfUe.Supi.String()))
	...
	case amfContext.UeContextReleaseUeContext:
		ran.Log.Info("Release UE Context: Release Ue Context", logger.SUPI(amfUe.Supi.String()))
		...
		if !amfUe.SecurityContextAvailable {
			ran.Log.Info("Valid Security is not exist for the UE, so deleting AmfUe Context", logger.SUPI(amfUe.Supi.String()))
			amf.DeregisterAndRemoveAMFUE(amfUe)
		}
	case amfContext.UeContextReleaseDueToNwInitiatedDeregistraion:
		ran.Log.Info("Release UE Context Due to Nw Initiated: Release Ue Context", logger.SUPI(amfUe.Supi.String()))
	...
	case amfContext.UeContextReleaseHandover:
		ran.Log.Info("Release UE Context : Release for Handover", logger.SUPI(amfUe.Supi.String()))
