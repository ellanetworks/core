...
func HandlePDUSessionResourceReleaseResponse(ctx context.Context, amf *amfContext.AMF, ran *amfContext.Radio, msg *ngapType.PDUSessionResourceReleaseResponse) {
	...
	if aMFUENGAPID == nil {
		ran.Log.Error("AMFUENGAPID IE (mandatory) is missing in PDUSessionResourceReleaseResponse")
		return
	}

	if rANUENGAPID == nil {
		ran.Log.Error("RANUENGAPID IE (mandatory) is missing in PDUSessionResourceReleaseResponse")
		return
	}
