...
// the second, etc.).
func decryptAndDecodeNasPdu(t *testing.T, ue *UeContext, nasPdu []byte, dlCountOffset uint32) *nas.Message {
	t.Helper()
...
func buildMobilityRegUeAndAMF(t *testing.T) (*UeContext, *FakeNGAPSender, *FakeSmf, *AMF) {
	t.Helper()
...
func TestMobilityReg_MultiSlice_AllowedNssaiContainsAllSlices(t *testing.T) {
	...
	amfInstance := New(dbInstance, nil, fakeSmf)
