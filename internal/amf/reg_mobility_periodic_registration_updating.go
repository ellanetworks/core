...
func HandleMobilityAndPeriodicRegistrationUpdating(ctx context.Context, amfInstance *AMF, ue *UeContext) error {
	...
	if len(ue.Pei) == 0 {
		...
		err := SendIdentityRequest(ctx, ranUe, nasMessage.MobileIdentity5GSTypeImei)
		if err != nil {
		...
		if conn.N1N2Message != nil {
			...
			if n2Info == nil {
				if len(suList.List) != 0 {
					nasPdu, err := BuildRegistrationAccept(amfInstance, ue, pduSessionStatus, reactivationResult, errPduSessionID, errCause, *operatorInfo.Guami.PlmnID)
					if err != nil {
						return err
					}
					...
					err := SendRegistrationAccept(ctx, amfInstance, ue, pduSessionStatus, reactivationResult, errPduSessionID, errCause, &ctxList, *operatorInfo.Guami.PlmnID, operatorInfo.Guami)
					if err != nil {
				...
				err := SendDLNASTransport(ctx, ranUe, nasMessage.PayloadContainerTypeN1SMInfo, n1Msg, requestData.PduSessionID, 0)
				if err != nil {
			...
			if n1Msg != nil {
				nasPdu, err = BuildDLNASTransport(ue, nasMessage.PayloadContainerTypeN1SMInfo, n1Msg, requestData.PduSessionID, nil)
				if err != nil {
					return err
				}
			}
