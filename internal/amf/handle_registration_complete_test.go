...
func setupRegistrationCompleteUE(t *testing.T) (*UeContext, *FakeNGAPSender) {
	t.Helper()
	...
	ue.NasConn().T3550 = NewTimer(5*time.Minute, 5, func(expireTimes int32) {}, func() {})
...
func TestHandleRegistrationComplete_WrongState_Error(t *testing.T) {
	testcases := []StateType{Deregistered, Authentication, Registered, SecurityMode}
...
func checkUERegistrationDataIsCleared(ue *UeContext) error {
