...
type UpdateInputs struct {
	...
	mi           []uint8
	expected_err error
	validate_ue  func(ue *UeContext) error
}

func emptyValidation(ue *UeContext) error {
	return nil
}

func newTestUe(integrityVerified bool, guti, oldGuti etsi.GUTI, tmsi etsi.TMSI) *UeContext {
	_ = integrityVerified
	ue := NewUeContext()
	ue.Guti = guti
	ue.OldGuti = oldGuti
...
func tmsiUe(integrityVerified bool, tmsi, oldTmsi etsi.TMSI) *UeContext {
	_ = integrityVerified
	ue := NewUeContext()
	ue.Tmsi = tmsi
	ue.OldTmsi = oldTmsi
...
func TestHandleIdentityResponse_InvalidStateError(t *testing.T) {
	testcases := []StateType{Deregistered, Registered, SecurityMode}
	...
	for _, tc := range testcases {
		t.Run(string(tc), func(t *testing.T) {
			...
			err := handleIdentityResponse(context.TODO(), New(nil, nil, nil), ue, &nasMessage.IdentityResponse{}, true)
			if err == nil {
...
func TestHandleIdentityResponse_ContextSetup_Error(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(fmt.Sprintf("%v", tc), func(t *testing.T) {
			supi := mustSUPIFromPrefixed("imsi-001019756139935")
			amfInstance := New(&FakeDBInstance{}, &FakeAusf{
				AvKgAka: &ausf.AuthResult{
					Rand: hex.EncodeToString(make([]byte, 16)),
					Autn: hex.EncodeToString(make([]byte, 16)),
				},
...
func TestHandleIdentityResponse_IdentityError(t *testing.T) {
	testcases := []StateType{Authentication, ContextSetup}
