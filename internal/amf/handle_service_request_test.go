...
func TestHandleServiceRequest_WrongStateError(t *testing.T) {
	testcases := []StateType{SecurityMode, Authentication, ContextSetup}
	for _, tc := range testcases {
		t.Run(string(tc), func(t *testing.T) {
			...
			err := handleServiceRequest(t.Context(), New(nil, nil, nil), ue, nil, true)
...
func TestHandleServiceRequest_ServiceTypeSignaling_ServiceAccept(t *testing.T) {
	...
	ue.NasConn().T3513 = NewTimer(6*time.Minute, 5, func(expireTimes int32) {}, func() {})
	...
	decoded, err := DecodeNASMessage(ue, resp.NasPdu)
	if err != nil {
...
func TestHandleServiceRequest_NASContainerServiceTypeSignaling_ServiceAccept(t *testing.T) {
	...
	ue.NasConn().T3565 = NewTimer(6*time.Minute, 5, func(expireTimes int32) {}, func() {})
	ue.ForceState(Registered)
	ue.Tai = ue.RanUe().Tai
	ue.SecurityContextAvailable = true
	ue.NgKsi.Ksi = 1
...
func TestHandleServiceRequest_NASContainerServiceTypeMT_N1N2Message_ExistingPDUSession_ServiceAccept(t *testing.T) {
	...
	amfInstance.T3555Cfg = TimerValue{Enable: true, ExpireTime: 5 * time.Minute, MaxRetryTimes: 5}
...
func TestHandleServiceRequest_OutOfRangePduSessionID_UplinkDataStatus(t *testing.T) {
	...
	ue.SmContextList[255] = &SmContext{Ref: "malicious-ref", Snssai: &snssai}
...
func TestHandleServiceRequest_OutOfRangePduSessionID_PDUSessionStatus(t *testing.T) {
	...
	ue.SmContextList[200] = &SmContext{Ref: "malicious-ref", Snssai: &snssai}
