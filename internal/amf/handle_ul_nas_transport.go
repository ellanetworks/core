...
func forward5GSMMessageToSMF(
	...
	if response.N1Msg != nil {
		...
		n1Msg, err = BuildDLNASTransport(ue, nasMessage.PayloadContainerTypeN1SMInfo, response.N1Msg, pduSessionID, nil)
		if err != nil {
...
func transport5GSMMessage(ctx context.Context, amfInstance *AMF, ue *UeContext, ulNasTransport *nasMessage.ULNASTransport) error {
...
// §5.4.5.3.1 case e)/f)).
func sendPayloadNotForwarded(ctx context.Context, ranUe *RanUe, pduSessionID uint8, smMessage []byte) error {
	if err := SendDLNASTransport(ctx, ranUe, nasMessage.PayloadContainerTypeN1SMInfo, smMessage, pduSessionID, nasMessage.Cause5GMMPayloadWasNotForwarded); err != nil {
		return fmt.Errorf("error sending downlink nas transport: %s", err)
	}
...
// forwarded.
func establishPDUSession(ctx context.Context, amfInstance *AMF, ue *UeContext, ranUe *RanUe, ulNasTransport *nasMessage.ULNASTransport, pduSessionID uint8, smMessage []byte) error {
	var (
	...
	if errResponse != nil {
		if sendErr := SendDLNASTransport(ctx, ranUe, nasMessage.PayloadContainerTypeN1SMInfo, errResponse, pduSessionID, 0); sendErr != nil {
...
func handleULNASTransport(ctx context.Context, amfInstance *AMF, ue *UeContext, msg *nasMessage.ULNASTransport, integrityVerified bool) error {
	if ue.GetState() != Registered {
		return fmt.Errorf("expected UE to be in state %s during UL NAS Transport, instead it was %s", Registered, ue.GetState())
	}
