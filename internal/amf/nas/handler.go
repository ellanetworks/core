...
import (
	...
	"github.com/ellanetworks/core/internal/amf/nas/security"
...
func HandleNAS(ue *context.RanUe, procedureCode int64, nasPdu []byte) {
	...
	if ue.AmfUe == nil {
		ue.AmfUe = security.FetchUeContextWithMobileIdentity(nasPdu)
		if ue.AmfUe == nil {
	...
	msg, err := security.Decode(ue.AmfUe, ue.Ran.AnType, nasPdu)
	if err != nil {
...
func DispatchMsg(amfUe *context.AmfUe, transInfo context.NasMsg) {
	...
	msg, err := security.Decode(amfUe, transInfo.AnType, transInfo.NasMsg)
	if err != nil {
