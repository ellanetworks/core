...
func sendServiceAccept(
	...
	if ranUe.UeContextRequest {
		...
		nasPdu, err := BuildServiceAccept(ue, pDUSessionStatus, reactivationResult, errPduSessionID, errCause)
		if err != nil {
		...
		err := SendServiceAccept(ctx, ranUe, pDUSessionStatus, reactivationResult, errPduSessionID, errCause)
		if err != nil {
...
// TS 24501 5.6.1
func handleServiceRequest(ctx context.Context, amfInstance *AMF, ue *UeContext, msg *nasMessage.ServiceRequest, integrityVerified bool) error {
	// Validate state before accessing RanUe — state checks are cheap and
	// independent of the RAN connection.
	state := ue.GetState()
	if state != Deregistered && state != Registered {
	...
	if state == Deregistered {
		...
		ranUe.ReleaseAction = UeContextN2NormalRelease
	...
	case nasMessage.ServiceTypeMobileTerminatedServices: // Triggered by Network
		...
		SendConfigurationUpdateCommand(ctx, amfInstance, ue, true)
