...
func handleRegistrationRequestMessage(ctx context.Context, amfInstance *AMF, ue *UeContext, registrationRequest *nasMessage.RegistrationRequest, integrityVerified bool) error {
	ranUe := ue.RanUe()
	if ranUe == nil {
	...
	if registrationRequest.NASMessageContainer != nil && integrityVerified {
		...
		if err != nil {
			...
			err1 := SendRegistrationReject(ctx, ranUe, nasMessage.Cause5GMMUEIdentityCannotBeDerivedByTheNetwork)
			if err1 != nil {
	...
	// Check TAI
	if !InTaiList(ue.Tai, operatorInfo.Tais) {
		metrics.RegistrationAttempt(metrics.RAT5G, getRegistrationType5GSName(conn.RegistrationType5GS), metrics.ResultReject)

		err := SendRegistrationReject(ctx, ranUe, nasMessage.Cause5GMMTrackingAreaNotAllowed)
		if err != nil {
	...
	if registrationRequest.UESecurityCapability == nil &&
		...
		err := SendRegistrationReject(ctx, ranUe, nasMessage.Cause5GMMProtocolErrorUnspecified)
		if err != nil {
			return fmt.Errorf("error sending registration reject: %v", err)
		}
...
func acceptRegistrationUESecurityCapability(ue *UeContext, received *nasType.UESecurityCapability) {
	conn := ue.NasConn()
	if conn == nil {
		return
	}
	...
	case nasMessage.RegistrationType5GSInitialRegistration,
		...
		ue.SetUESecurityCapability(received, MintAuthProofForRegistrationRequest())
		return
	}
...
func handleRegistrationRequest(ctx context.Context, amfInstance *AMF, ue *UeContext, msg *nas.GmmMessage, integrityVerified bool) error {
	state := ue.GetState()

	switch state {
	case Deregistered, Registered, Authentication:
		...
		if err != nil {
			...
			err := SendRegistrationReject(ctx, ranUe, nasMessage.Cause5GMMUEIdentityCannotBeDerivedByTheNetwork)
			if err != nil {
				return fmt.Errorf("error sending registration reject: %v", err)
			}
