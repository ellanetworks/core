...
func (db *Database) UpdateConfig(ctx context.Context, cfg client.SyncConfig) error {
	...
	if err = syncPolicies(ctx, tx, cfg.Policies); err != nil {
		span.RecordError(err)
		return fmt.Errorf("sync policies: %w", err)
	}

	if err = syncSubscribers(ctx, tx, cfg.Subscribers); err != nil {
		span.RecordError(err)
...
// Policies reference data networks by ID. The desired config uses the fleet-assigned
// data_network_id which must match the local DB ID after data network sync.
func syncPolicies(ctx context.Context, tx *sql.Tx, desired []client.Policy) error {
	...
	for _, d := range desired {
		...
		if cur, ok := existing[d.Name]; ok {
			...
			_, err := tx.ExecContext(ctx,
				...
				d.Name, d.BitrateUplink, d.BitrateDownlink, d.Var5qi, d.Arp, d.DataNetworkID,
			)
			if err != nil {
...
// Subscribers are keyed by IMSI.
func syncSubscribers(ctx context.Context, tx *sql.Tx, desired []client.Subscriber) error {
	rows, err := tx.QueryContext(ctx,
	...
	for _, d := range desired {
		...
		if cur, ok := existing[d.Imsi]; ok {
			...
			_, err := tx.ExecContext(ctx,
				...
				d.Imsi, "000000000000", d.PermanentKey, d.Opc, d.PolicyID,
			)
			if err != nil {
