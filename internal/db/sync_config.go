...
// newUUIDv7 generates a fresh UUIDv7 string for entities created during
// fleet sync. Keep all generation in one place so the error path is
// uniform.
...
// here. Per-node network state (NAT, flow accounting, N3, routes, BGP)
// is applied separately via UpdateNodeConfig — those tables are
// local-only and each node owns its own copy.
//
...
func (db *Database) UpdateNodeConfig(ctx context.Context, cfg client.SyncConfig) error {
	...
	if err := db.applyNodeConfig(ctx, &cfg); err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, err.Error())
...
func (db *Database) syncHomeNetworkKeys(ctx context.Context, desired []client.HomeNetworkKey) error {
	...
	for _, k := range existing {
		...
		if _, err := db.applyDeleteHomeNetworkKey(ctx, &stringPayload{Value: k.ID}); err != nil {
...
func (db *Database) syncPoliciesUpserts(ctx context.Context, desired []client.Policy) error {
	...
	profileID := make(map[string]string, len(profiles))
	...
	sliceID := make(map[string]string, len(slices))
	...
	dnID := make(map[string]string, len(dataNetworks))
...
func (db *Database) syncRoutes(ctx context.Context, desired []client.Route) error {
	...
	for key, id := range have {
		...
		if err := db.applyDeleteRoute(ctx, &int64Payload{Value: id}); err != nil {
...
func (db *Database) syncNetworkRules(ctx context.Context, desired []client.NetworkRule) error {
	...
	policyIDByName := make(map[string]string, len(policies))
	for _, p := range policies {
		policyIDByName[p.Name] = p.ID
	}
	...
	for _, p := range policies {
		if _, err := db.applyDeleteNetworkRulesByPolicy(ctx, &stringPayload{Value: p.ID}); err != nil {
...
func (db *Database) syncBGPSettings(ctx context.Context, desired client.BGPSettings) error {
	...
	if err := db.applyUpdateBGPSettings(ctx, s); err != nil {
...
// syncBGPPeersAndPrefixes reconciles this node's BGP peers against the
// desired set Fleet has scoped to this node. Import prefixes are grouped
// by peer address and replaced atomically per peer.
func (db *Database) syncBGPPeersAndPrefixes(ctx context.Context, desiredPeers []client.BGPPeer, desiredPrefixes []client.BGPImportPrefix) error {
	...
	for addr, cur := range have {
		...
		if err := db.applyDeleteBGPPeer(ctx, &intPayload{Value: cur.ID}); err != nil {
	...
	for _, d := range desiredPeers {
		...
		if err := db.applyUpdateBGPPeer(ctx, upd); err != nil {
...
// replacePeerImportPrefixes atomically swaps a peer's import prefixes
// via the local-only delete-then-insert path on bgp_import_prefixes.
func (db *Database) replacePeerImportPrefixes(ctx context.Context, peerID int, prefixes []BGPImportPrefix) error {
	...
	return db.SetImportPrefixesForPeer(ctx, peerID, prefixes)
}
