...
func (db *Database) InsertRadioEvent(ctx context.Context, radioEvent *dbwriter.RadioEvent) error {
	...
	err := db.local.Query(ctx, db.insertRadioEventStmt, radioEvent).Run()
	if err != nil {
...
func (db *Database) ListRadioEvents(ctx context.Context, page int, perPage int, filters *RadioEventFilters) ([]dbwriter.RadioEvent, int, error) {
	...
	err := db.local.Query(ctx, db.listRadioEventsStmt, args, filters).GetAll(&logs, &counts)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			...
			countErr := db.local.Query(ctx, db.countRadioEventsStmt, filters).Get(&fallbackCount)
			if countErr != nil {
				return nil, 0, nil
			}
...
func (db *Database) DeleteOldRadioEvents(ctx context.Context, days int) error {
	...
	err := db.local.Query(ctx, db.deleteOldRadioEventsStmt, args).Run()
	if err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, "query failed")
...
func (db *Database) ClearRadioEvents(ctx context.Context) error {
	...
	err := db.local.Query(ctx, db.deleteAllRadioEventsStmt).Run()
	if err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, "query failed")
...
func (db *Database) GetRadioEventByID(ctx context.Context, id int) (*dbwriter.RadioEvent, error) {
	...
	err := db.local.Query(ctx, db.getRadioEventByIDStmt, log).Get(&log)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			span.SetStatus(codes.Ok, "no rows")
