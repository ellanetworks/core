...
func migrateV20ClusterMembers(ctx context.Context, tx *sql.Tx) error {
	stmts := []string{
		...
		fmt.Sprintf(`INSERT INTO %s_new (nodeID, apiAddress, binaryVersion, amfPointer, drainState, drainUpdatedAt)
			SELECT CAST(nodeID AS TEXT), apiAddress, binaryVersion, nodeID, drainState, drainUpdatedAt FROM %s`,
