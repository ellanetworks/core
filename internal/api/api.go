...
func Start(dbInstance *db.Database, cfg config.Config, upf server.UPFReloader, embedFS fs.FS, registerExtraRoutes func(mux *http.ServeMux)) error {
	...
	go func() {
		httpAddr := ":" + strconv.Itoa(cfg.Interfaces.API.Port)
	...
	logger.APILog.Info("API server started", zap.String("scheme", string(scheme)), zap.String("address", fmt.Sprintf("%s://127.0.0.1:%d", scheme, cfg.Interfaces.API.Port)))
