...
func TestNetworkInteraces_EndToEnd(t *testing.T) {
	...
	t.Run("5. Validate that N3 external address is updated", func(t *testing.T) {
		statusCode, resp, err := listNetworkInterfaces(ts.URL, client, token)
		if err != nil {
			t.Fatalf("couldn't list network interfaces: %s", err)
		}
		...
		if resp.Result.N3.ExternalAddress != "example.com" {
			t.Fatalf("unexpected N3 interface external address: %s", resp.Result.N3.ExternalAddress)
		}
	})

	t.Run("6. Update N3 external address to empty", func(t *testing.T) {
	...
	t.Run("7. Validate that N3 external address is updated", func(t *testing.T) {
		statusCode, resp, err := listNetworkInterfaces(ts.URL, client, token)
		if err != nil {
			t.Fatalf("couldn't list network interfaces: %s", err)
		}
