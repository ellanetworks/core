...
type GetOperatorSPNResponseResult struct {
...
func TestApiOperatorEndToEnd(t *testing.T) {
	...
	t.Run("8. Get operator tracking via GET /api/v1/operator", func(t *testing.T) {
	...
	t.Run("11. Get operator Id via GET /api/v1/operator", func(t *testing.T) {
		statusCode, response, err := getOperator(env.Server.URL, client, token)
		if err != nil {
			t.Fatalf("couldn't get operator: %s", err)
		}
