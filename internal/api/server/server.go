...
func NewHandler(cfg HandlerConfig) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/ran/radios/{ref}", Authenticate(jwtSecret, dbInstance, Authorize(PermReadRadio, GetRadio(amfInstance, mmeInstance))).ServeHTTP)
	mux.HandleFunc("DELETE /api/v1/ran/radios/{ref}", Authenticate(jwtSecret, dbInstance, Authorize(PermForgetRadio, ForgetRadio(amfInstance, mmeInstance))).ServeHTTP)
	...
	mux.HandleFunc("POST /api/v1/cluster/pki/join-tokens", Authenticate(jwtSecret, dbInstance, Authorize(PermManageCluster, LeaderOnly(dbInstance, pkiAdminEndpoint(func(svc *pkiissuer.Service) http.Handler {
