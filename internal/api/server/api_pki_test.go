...
// TestPKIAdminEndpoints_State checks GET /api/v1/cluster/pki/state
// returns populated data after bootstrap.
func TestPKIAdminEndpoints_State(t *testing.T) {
	env, err := setupServer(filepath.Join(t.TempDir(), "ella.db"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := env.DB.UpdateOperatorClusterID(context.Background(), "cluster-state"); err != nil {
		t.Fatal(err)
	}
	...
	req, _ := http.NewRequestWithContext(context.Background(), http.MethodGet,
		env.Server.URL+"/api/v1/cluster/pki/state", nil)
	req.Header.Set("Authorization", "Bearer "+admin)
	...
	var env2 struct {
		Result server.PKIStateResponse `json:"result"`
	}
	...
	if env2.Result.ClusterID != "cluster-state" {
		t.Fatalf("clusterID = %q", env2.Result.ClusterID)
	}

	if len(env2.Result.Roots) != 1 {
		t.Fatalf("roots = %v", env2.Result.Roots)
	}

	if len(env2.Result.Intermediates) != 1 {
		t.Fatalf("intermediates = %v", env2.Result.Intermediates)
	}
}
