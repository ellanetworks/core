...
func leaderOnlyResult(t *testing.T, f *fakeLeadership) (int, raft.NotLeaderBody, bool) {
	t.Helper()
...
func TestLeaderOnlyRejectsFollower(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if body.LeaderAddress != tc.wantRaftAddr {
				t.Fatalf("got leaderAddress %q, want %q", body.LeaderAddress, tc.wantRaftAddr)
			}
		})
	}
}
