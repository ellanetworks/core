...
func TestBuildPDUSessionModificationCommand_AmbrAndQoS(t *testing.T) {
	...
	ambrIE := *m.SessionAMBR

	// 200 Mbps encodes as unit 1 Mbps (0x06), value 200.
	if ambrIE.UplinkUnit != fgs.SessionAMBRUnit1Mbps || ambrIE.Uplink != 200 {
		t.Errorf("uplink AMBR = %d unit %d", ambrIE.Uplink, ambrIE.UplinkUnit)
	}

	if ambrIE.DownlinkUnit != fgs.SessionAMBRUnit1Mbps || ambrIE.Downlink != 200 {
		t.Errorf("downlink AMBR = %d unit %d", ambrIE.Downlink, ambrIE.DownlinkUnit)
	}
