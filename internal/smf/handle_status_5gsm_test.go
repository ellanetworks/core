...
func build5GSMStatus(pduSessionID, pti uint8, cause fgs.GSMCause) []byte {
	return []byte{uint8(fgs.EPD5GSM), pduSessionID, pti, uint8(fgs.MsgGSMStatus), uint8(cause)}
}
...
func TestStatus5GSM_PTIMismatchOnEstablishmentAcceptReleasesLocally(t *testing.T) {
	...
	ref, rsp, err := s.CreateSmContext(ctx, testSUPI(), 1, testDNN, testSnssai, fgs.RequestTypeInitialRequest, buildPDUSessionEstRequestWithPTI(10), 0)
	if err != nil {
...
func TestStatus5GSM_PTIMismatchOnAnotherPTIKeepsSession(t *testing.T) {
	...
	ref, _, err := s.CreateSmContext(ctx, testSUPI(), 1, testDNN, testSnssai, fgs.RequestTypeInitialRequest, buildPDUSessionEstRequestWithPTI(10), 0)
	if err != nil {
		t.Fatalf("CreateSmContext failed: %v", err)
	}
