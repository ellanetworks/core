...
func TestBuildQosRules(t *testing.T) {
	...
	if bytes, err := qosRules.MarshalBinary(); err != nil {
		...
		expectedBytes1 := []byte{0x01, 0x00, 0x37, 0x32, 0x31, 0x18, 0x10, 0x01, 0x01, 0x01, 0x01, 0xff, 0xff, 0xff, 0xff, 0x50, 0x03, 0xe8, 0x11, 0x02, 0x02, 0x02, 0x02, 0xff, 0xff, 0xff, 0xff, 0x40, 0x07, 0xd0, 0x32, 0x18, 0x10, 0x03, 0x03, 0x03, 0x03, 0xff, 0xff, 0xff, 0xff, 0x50, 0x0b, 0xb8, 0x11, 0x04, 0x04, 0x04, 0x04, 0xff, 0xff, 0xff, 0xff, 0x40, 0x0f, 0xa0, 0xc8, 0x05, 0x01, 0x00, 0x06, 0x31, 0x31, 0x01, 0x01, 0xff, 0x08}
		expectedBytes2 := []byte{0x01, 0x00, 0x37, 0x32, 0x31, 0x18, 0x10, 0x01, 0x01, 0x01, 0x01, 0xff, 0xff, 0xff, 0xff, 0x50, 0x03, 0xe8, 0x11, 0x02, 0x02, 0x02, 0x02, 0xff, 0xff, 0xff, 0xff, 0x40, 0x07, 0xd0, 0x32, 0x18, 0x10, 0x03, 0x03, 0x03, 0x03, 0xff, 0xff, 0xff, 0xff, 0x50, 0x0b, 0xb8, 0x11, 0x04, 0x04, 0x04, 0x04, 0xff, 0xff, 0xff, 0xff, 0x40, 0x0f, 0xa0, 0xc8, 0x05, 0x01, 0x00, 0x06, 0x31, 0x31, 0x01, 0x01, 0xff, 0x09}
...
func TestBuildAddDefaultQosRule(t *testing.T) {
	qosFlow, err := qos.BuildAddDefaultQosRule(1)
	if err != nil {
		t.Fatalf("Error building default QoS Flow: %v", err)
	}
...
func TestBuildAddDefaultQosRule_LargeQFI(t *testing.T) {
	_, err := qos.BuildAddDefaultQosRule(64)
	if err == nil {
		t.Errorf("Expected error building default QoS Flow, got none")
	}
}
