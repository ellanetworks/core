...
func CreateSmContext(ctx ctxt.Context, request models.PostSmContextsRequest) (string, *models.PostSmContextsErrorResponse, error) {
	...
	location, pco, pduSessionType, estAcceptCause5gSMValue, dnnInfo, pduAddress, errRsp, err := producer.HandlePDUSessionSMContextCreate(ctx, request, smContext)
	if err != nil {
	...
	if err != nil {
		if smContext != nil {
			err := producer.SendPduSessN1N2Transfer(ctx, smContext, pco, pduSessionType, estAcceptCause5gSMValue, dnnInfo, pduAddress, false)
			if err != nil {
	...
	err = producer.SendPduSessN1N2Transfer(ctx, smContext, pco, pduSessionType, estAcceptCause5gSMValue, dnnInfo, pduAddress, true)
	if err != nil {
		logger.SmfLog.Error("error transferring n1 n2", zap.Error(err))
		return "", nil, fmt.Errorf("failed to create SM Context: %v", err)
	}
