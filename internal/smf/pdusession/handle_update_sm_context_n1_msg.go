...
func handleUpdateN1Msg(ctx context.Context, n1Msg []byte, smContext *smfContext.SMContext) (*models.UpdateSmContextResponse, bool, error) {
	...
	logger.WithTrace(ctx, logger.SmfLog).Debug("Update SM Context Request N1SmMessage", zap.String("supi", smContext.Supi.String()), zap.Uint8("pduSessionID", smContext.PDUSessionID))

	switch m.GsmHeader.GetMessageType() {
	case nas.MsgTypePDUSessionReleaseRequest:
		logger.WithTrace(ctx, logger.SmfLog).Info("N1 Msg PDU Session Release Request received", zap.String("supi", smContext.Supi.String()), zap.Uint8("pduSessionID", smContext.PDUSessionID))
	...
	default:
		logger.WithTrace(ctx, logger.SmfLog).Warn("N1 Msg type not supported in SM Context Update", zap.Uint8("MessageType", m.GsmHeader.GetMessageType()), zap.String("supi", smContext.Supi.String()), zap.Uint8("pduSessionID", smContext.PDUSessionID))
		return nil, false, nil
	}
}
