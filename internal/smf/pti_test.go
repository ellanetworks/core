...
func buildPDUSessionReleaseComplete(pduSessionID, pti uint8) []byte {
	return []byte{uint8(fgs.EPD5GSM), pduSessionID, pti, uint8(fgs.MsgPDUSessionReleaseComplete)}
}

func status5GSMCause(t *testing.T, raw []byte) fgs.GSMCause {
	t.Helper()
...
func TestCreateSmContext_AlwaysOnRequested_IndicationNotAllowed(t *testing.T) {
	...
	ref, rsp, err := s.CreateSmContext(ctx, supi, 1, testDNN, testSnssai, fgs.RequestTypeInitialRequest, buildPDUSessionEstRequestAlwaysOn(), 0)
	if err != nil {
	...
	if *acc.AlwaysOn {
		t.Error("APSI = true, want false (always-on not allowed)")
	}
}
...
func TestCreateSmContext_UnassignedPTI_Status81(t *testing.T) {
	...
	ref, rsp, err := s.CreateSmContext(ctx, supi, 1, testDNN, testSnssai, fgs.RequestTypeInitialRequest, buildPDUSessionEstRequestWithPTI(0), 0)
	if err != nil {
		t.Fatalf("CreateSmContext failed: %v", err)
	}
...
func TestCreateSmContext_ReservedPTI_Ignored(t *testing.T) {
	...
	ref, rsp, err := s.CreateSmContext(ctx, supi, 1, testDNN, testSnssai, fgs.RequestTypeInitialRequest, buildPDUSessionEstRequestWithPTI(0xff), 0)
	if err != nil {
		t.Fatalf("CreateSmContext failed: %v", err)
	}
