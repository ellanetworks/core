...
// TestStaticIPChanged covers the reconcile comparison for an EPS session: a
...
// TestReconcileSession_StaticIPCreatedReleasesSession asserts the 5G reconcile
// path releases the session (cause #39) when a static reservation is created for
// a session currently on a dynamic address.
func TestReconcileSession_StaticIPCreatedReleasesSession(t *testing.T) {
	...
	if err := reconcileWithPolicy(ctx, s, pcf, ref, nil); err != nil {
