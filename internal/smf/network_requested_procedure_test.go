...
func buildPDUSessionModificationComplete(pduSessionID, pti uint8) []byte {
	return []byte{uint8(fgs.EPD5GSM), pduSessionID, pti, uint8(fgs.MsgPDUSessionModificationComplete)}
}

func buildPDUSessionModificationCommandReject(pduSessionID, pti uint8) []byte {
	return []byte{uint8(fgs.EPD5GSM), pduSessionID, pti, uint8(fgs.MsgPDUSessionModificationCmdReject), uint8(fgs.GSMCauseRequestRejectedUnspecified)}
}
