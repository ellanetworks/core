...
func TestCreateSmContext_DNNNotFound_PreservesExistingSession(t *testing.T) {
	store, upf, amfCb := defaultFakes()
	s := newTestSMF(store, upf, amfCb)
	ctx := context.Background()
	supi := testSUPI()
	...
	// Establish a working session on PDU session ID 1.
	ref, _, err := s.CreateSmContext(ctx, supi, 1, testDNN, testSnssai, n1Msg)
	if err != nil {
		t.Fatalf("first CreateSmContext failed: %v", err)
	}

	existingSession := s.GetSession(ref)
	if existingSession == nil {
		t.Fatal("expected session in pool after successful create")
	}

	// Now simulate a request for a non-existent DNN reusing the same PDU session ID.
	// This must reject without destroying the existing "internet" session.
	store.mu.Lock()
	store.dnnInfo = nil // make DNN lookup fail
	store.mu.Unlock()
	...
	// The original session must still be alive with the same reference.
	survivingSession := s.GetSession(ref)
	if survivingSession == nil {
		t.Fatal("existing session was destroyed by a failed DNN request — regression")
	}

	if survivingSession != existingSession {
		t.Fatal("session was replaced instead of preserved")
	}
}

func TestCreateSmContext_DNNNotFound_NoIPRelease(t *testing.T) {
	store, upf, amfCb := defaultFakes()
	store.dnnInfo = nil // DNN does not exist
	s := newTestSMF(store, upf, amfCb)
	ctx := context.Background()
	supi := testSUPI()
	n1Msg := buildPDUSessionEstRequest()
	...
	if len(store.releasedIPs) != 0 {
		t.Fatalf("DNN pre-check failure should not trigger any IP release, got %v", store.releasedIPs)
	}
}
