...
func aucSQN(opc, k, auts, rand []byte) ([]byte, []byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("milenage F2345 error: %w", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("milenage F1 error: %w", err)
	}
...
func CreateAuthData(authInfoRequest models.AuthenticationInfoRequest, ueSuci string) (*models.AuthenticationInfoResult, error) {
	...
	supi, err := suci.ToSupi(ueSuci, hnPrivateKey)
	if err != nil {
	...
	*/
	...
	if authSubs.Milenage.Op != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("error decoding op: %w", err)
		}
		hasOP = true
	}
	...
	if authSubs.Opc != nil && authSubs.Opc.OpcValue != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("error decoding opc: %w", err)
		}
		hasOPC = true
	}
	...
	if !hasOPC && hasOP {
		...
		if err != nil {
			return nil, fmt.Errorf("error generating OPC: %w", err)
		}
	}
	...
	if authInfoRequest.ResynchronizationInfo != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("could not get SQNms and macS: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("milenage F1 error: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("milenage F2345 error: %w", err)
	}
