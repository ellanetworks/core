...
// 	response, err := HandlePfcpSessionEstablishmentRequest(seReq)
// 	if err != nil {
// 		t.Errorf("Error handling Session Establishment Request: %s", err)
// 	}

// 	// Checking TEID for each PDR
// 	logger.UpfLog.Infof("seRes.CreatedPDR len: %d", len(response.CreatedPDR))
// 	if len(response.CreatedPDR) != 2 {
// 		t.Errorf("Unexpected count TEIDs: got %d, expected %d", len(response.CreatedPDR), 2)
// 	}

// 	for _, pdr := range response.CreatedPDR {
...
// 		logger.UpfLog.Infof("seRes.CreatedPDR len: %d", len(response.CreatedPDR))
// 		if len(response.CreatedPDR) != 1 {
// 			t.Errorf("Unexpected count PRD's: got %d, expected %d", len(response.CreatedPDR), 1)
// 		}
