...
func HandlePfcpSessionEstablishmentRequest(msg *message.SessionEstablishmentRequest) (*message.SessionEstablishmentResponse, error) {
	...
	association, ok := conn.NodeAssociations[config.Conf.SmfAddress]
	if !ok {
	...
	err = func() error {
		...
		for _, far := range msg.CreateFAR {
			...
			internalId, err := bpfObjects.NewUplinkFar(farInfo)
			if err == nil {
			...
			internalId, err = bpfObjects.NewDownlinkFar(farInfo)
			if err == nil {
				session.NewFar(farid, internalId, farInfo)
			} else {
				logger.UpfLog.Infof("Can't put FAR: %s", err.Error())
		...
		for _, qer := range msg.CreateQER {
			...
			internalId, err := bpfObjects.NewUplinkQer(qerInfo)
			if err == nil {
			...
			internalId, err = bpfObjects.NewDownlinkQer(qerInfo)
			if err == nil {
				session.NewQer(qerId, internalId, qerInfo)
			} else {
				logger.UpfLog.Infof("Can't put QER: %s", err.Error())
	...
	conn.NodeAssociations[config.Conf.SmfAddress] = association
...
func HandlePfcpSessionDeletionRequest(msg *message.SessionDeletionRequest) (*message.SessionDeletionResponse, error) {
	...
	for id := range session.FARs {
		...
		// 	return message.NewSessionDeletionResponse(0, 0, 0, msg.Sequence(), 0, newIeNodeID(conn.nodeId), ie.NewCause(ie.CauseRuleCreationModificationFailure)), err
		// }
		err := bpfObjects.DeleteUplinkFar(id)
		if err != nil {
			return message.NewSessionDeletionResponse(0, 0, 0, msg.Sequence(), 0, newIeNodeID(conn.nodeId), ie.NewCause(ie.CauseRuleCreationModificationFailure)), err
		}
		err = bpfObjects.DeleteDownlinkFar(id)
		if err != nil {
			return message.NewSessionDeletionResponse(0, 0, 0, msg.Sequence(), 0, newIeNodeID(conn.nodeId), ie.NewCause(ie.CauseRuleCreationModificationFailure)), err
		}
	}
	for id := range session.QERs {
		err := bpfObjects.DeleteUplinkQer(id)
		if err != nil {
			return message.NewSessionDeletionResponse(0, 0, 0, msg.Sequence(), 0, newIeNodeID(conn.nodeId), ie.NewCause(ie.CauseRuleCreationModificationFailure)), err
		}
		err = bpfObjects.DeleteDownlinkQer(id)
		if err != nil {
			return message.NewSessionDeletionResponse(0, 0, 0, msg.Sequence(), 0, newIeNodeID(conn.nodeId), ie.NewCause(ie.CauseRuleCreationModificationFailure)), err
		}
	}
...
func HandlePfcpSessionModificationRequest(msg *message.SessionModificationRequest) (*message.SessionModificationResponse, error) {
	...
	if !ok {
		...
		return message.NewSessionModificationResponse(0, 0, msg.SEID(), msg.Sequence(), 0, newIeNodeID(conn.nodeId), ie.NewCause(ie.CauseNoEstablishedPFCPAssociation)), nil
	}

	logger.UpfLog.Infof("Finding session %d", msg.SEID())
	session, ok := association.Sessions[msg.SEID()]
	if !ok {
	...
	err := func() error {
		...
		for _, far := range msg.UpdateFAR {
			...
			err = bpfObjects.UpdateUplinkFar(sFarInfo.GlobalId, sFarInfo.FarInfo)
			if err != nil {
				logger.UpfLog.Infof("Can't update FAR: %s", err.Error())
			}
			err = bpfObjects.UpdateDownlinkFar(sFarInfo.GlobalId, sFarInfo.FarInfo)
			if err != nil {
				logger.UpfLog.Infof("Can't update FAR: %s", err.Error())
			}
		}
		...
		for _, far := range msg.RemoveFAR {
			...
			err := bpfObjects.DeleteUplinkFar(sFarInfo.GlobalId)
			if err != nil {
				logger.UpfLog.Infof("Can't remove FAR: %s", err.Error())
			}
			err = bpfObjects.DeleteDownlinkFar(sFarInfo.GlobalId)
			if err != nil {
				logger.UpfLog.Infof("Can't remove FAR: %s", err.Error())
			}
		}
		...
		for _, qer := range msg.UpdateQER {
			...
			// 	return err
			// }
			err = bpfObjects.UpdateUplinkQer(sQerInfo.GlobalId, sQerInfo.QerInfo)
			if err != nil {
				logger.UpfLog.Infof("Can't update QER: %s", err.Error())
			}
			err = bpfObjects.UpdateDownlinkQer(sQerInfo.GlobalId, sQerInfo.QerInfo)
			if err != nil {
				logger.UpfLog.Infof("Can't update QER: %s", err.Error())
			}
		}
		...
		for _, qer := range msg.RemoveQER {
			...
			err = bpfObjects.DeleteUplinkQer(sQerInfo.GlobalId)
			if err != nil {
				logger.UpfLog.Infof("Can't remove QER: %s", err.Error())
			}
			err = bpfObjects.DeleteDownlinkQer(sQerInfo.GlobalId)
			if err != nil {
				logger.UpfLog.Infof("Can't remove QER: %s", err.Error())
			}
		}
