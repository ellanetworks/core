...
static __always_inline __u8 match_sdf_filter_ipv4(
	const struct packet_context *ctx, const struct sdf_filter *sdf)
{
	if (!ctx || !ctx->ip4 || !sdf)
		return 0;

	const struct iphdr *ip4 = ctx->ip4;
	__u8 packet_protocol = get_sdf_protocol(
		ip4->protocol); // TODO: convert protocol in golang part
	__u16 packet_src_port = 0;
	__u16 packet_dst_port = 0;
	if (ctx->udp) {
		packet_src_port = bpf_ntohs(
			ctx->udp->source); // TODO: convert port in golang part
		packet_dst_port = bpf_ntohs(
			ctx->udp->dest); // TODO: convert port in golang part
	} else if (ctx->tcp) {
		packet_src_port = bpf_ntohs(
			ctx->tcp->source); // TODO: convert port in golang part
		packet_dst_port = bpf_ntohs(
			ctx->tcp->dest); // TODO: convert port in golang part
	}
	...
	upf_printk("SDF: filter source ip: %pI4, destination ip: %pI4",
		   &sdf_src_ip, &sdf_dst_ip);
	upf_printk(
	...
	upf_printk(
		"SDF: filter destination port lower bound: %u, destination port upper bound: %u",
		sdf->dst_port.lower_bound, sdf->dst_port.upper_bound);

	upf_printk("SDF: packet protocol: %u", packet_protocol);
	upf_printk("SDF: packet source ip: %pI4, destination ip: %pI4",
		   &ip4->saddr, &ip4->daddr);
	upf_printk("SDF: packet source port: %u, destination port: %u",
	...
	upf_printk(
		"Packet with source ip: %pI4, destination ip: %pI4 matches SDF filter",
		&ip4->saddr, &ip4->daddr);

	return 1;
}

static __always_inline __u8 match_sdf_filter_ipv6(
	const struct packet_context *ctx, const struct sdf_filter *sdf)
{
	...
	upf_printk("SDF: filter protocol: %u", sdf->protocol);
	upf_printk("SDF: filter source ip: %pI6c, destination ip: %pI6c",
		   &sdf_src_ip, &sdf_dst_ip);
	upf_printk(
		"SDF: filter source port lower bound: %u, source port upper bound: %u",
		sdf->src_port.lower_bound, sdf->src_port.upper_bound);
	upf_printk(
		...
		sdf->dst_port.lower_bound, sdf->dst_port.upper_bound);
	upf_printk(
		"SDF: filter source address mask: %pI4, destination address mask: %pI4",
		&sdf->dst_addr.mask, &sdf->dst_addr.mask);

	upf_printk("SDF: packet protocol: %u", packet_protocol);
	upf_printk("SDF: packet source ip: %pI6c, destination ip: %pI6c",
	...
	upf_printk(
		"SDF: packet with source ip:%pI6c, destination ip:%pI6c matches SDF filter",
		&packet_src_ip_128, &packet_dst_ip_128);
