...
static __always_inline __u8 match_sdf_filter_ipv4(const struct n6_packet_context *ctx, const struct sdf_filter *sdf)
{
    if (!ctx || !ctx->ip4 || !sdf)
        return 0;
    ...
    if ((sdf->protocol != 1 && sdf->protocol != packet_protocol) || ((ip4->saddr & sdf_src_mask) != sdf_src_ip) || ((ip4->daddr & sdf_dst_mask) != sdf_dst_ip) || (packet_src_port < sdf->src_port.lower_bound || packet_src_port > sdf->src_port.upper_bound) || (packet_dst_port < sdf->dst_port.lower_bound || packet_dst_port > sdf->dst_port.upper_bound))
    {
        return 0;
    }
...
static __always_inline __u8 match_sdf_filter_ipv6(const struct n6_packet_context *ctx, const struct sdf_filter *sdf)
{
