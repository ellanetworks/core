...
func TestCreatedPDRsToResponse_UplinkPDR(t *testing.T) {
	...
	result := createdPDRsToResponse(pdrs, n3Addr, netip.Addr{})
	...
	if result[0].N3IPv4 != n3Addr {
		t.Errorf("N3IPv4: got %v, want %v", result[0].N3IPv4, n3Addr)
	}
}
...
func TestCreatedPDRsToResponse_MixedPDRs(t *testing.T) {
	...
	if result[0].TEID != 100 || result[0].N3IPv4 != n3Addr {
		t.Errorf("PDR 1: got TEID=%d N3IPv4=%v, want TEID=100 N3IPv4=%v", result[0].TEID, result[0].N3IPv4, n3Addr)
	}
}

func TestCreatedPDRsToResponse_DualStack(t *testing.T) {
	...
	result := createdPDRsToResponse(pdrs, n3v4, n3v6)

	if len(result) != 1 {
		t.Fatalf("expected 1 CreatedPDR, got %d", len(result))
	}

	if result[0].N3IPv4 != n3v4 {
		t.Errorf("N3IPv4: got %v, want %v", result[0].N3IPv4, n3v4)
	}

	if result[0].N3IPv6 != n3v6 {
		t.Errorf("N3IPv6: got %v, want %v", result[0].N3IPv6, n3v6)
	}
}
