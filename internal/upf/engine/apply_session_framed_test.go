...
// TestApplySkipsMismatchedFamilyFramedRoute asserts that a framed route whose
// family the session does not carry (here an IPv6 route on an IPv4-only
// session) is skipped, not fatal: a dormant cross-family route must not deny
// the UE all connectivity (TS 23.501 §5.6.14). Requires root.
func TestApplySkipsMismatchedFamilyFramedRoute(t *testing.T) {
	if os.Geteuid() != 0 {
