...
import { registerFleet, getFleetURL, updateFleetURL } from "@/queries/fleet";
...
const ConfigureFleetModal: React.FC<ConfigureFleetModalProps> = ({
  ...
  useEffect(() => {
    if (open && accessToken) {
      getFleetURL(accessToken)
        .then((resp) => {
          if (resp.url) setFleetURL(resp.url);
        })
        .catch(() => {});
    }
  }, [open, accessToken]);
  ...
  const handleSubmit = async () => {
    ...
    if (!fleetURL.trim()) return;
    if (!activationToken.trim()) return;
    setLoading(true);
    setAlert({ message: "" });
    try {
      await updateFleetURL(accessToken, fleetURL.trim());
    ...
    >
      ...
      <DialogContent dividers>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Enter the Ella Fleet server address and paste the activation token to
        ...
        <TextField
          ...
          placeholder="https://fleet.example.com:5003"
          margin="normal"
          disabled={loading}
        />
        <TextField
      ...
      <DialogActions>
        ...
        <Button
          ...
          disabled={!fleetURL.trim() || !activationToken.trim() || loading}
        >
