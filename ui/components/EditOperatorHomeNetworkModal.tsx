...
const schema = yup.object().shape({
    privateKey: yup
        .string()
        .matches(/^[a-fA-F0-9]{64}$/, "Private Key must be a 64-character hexadecimal string.")
        .required("Private Key is required."),
});

const EditOperatorHomeNetworkModal: React.FC<EditOperatorHomeNetworkModalProps> = ({
    open,
    onClose,
    onSuccess,
}) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await updateOperatorHomeNetwork(cookies.user_token, formValues.privateKey);
            onClose();
            onSuccess();
        } catch (error: any) {
            const errorMessage = error?.message || "Unknown error occurred.";
            setAlert({ message: `Failed to update operator home network information: ${errorMessage}` });
        } finally {
            setLoading(false);
        }
    };
        ...
        >
            ...
            <DialogContent dividers>
                ...
                <DialogContentText id="alert-dialog-slide-description">
                    The Home Network Private Key ensures IMSI privacy. User Equipment (UE) devices will use the public key to encrypt the IMSI before sending it to the network. The network will then use the private key to decrypt the IMSI.
                </DialogContentText>
                <TextField
                    fullWidth
                    label="Private Key"
