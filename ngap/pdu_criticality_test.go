...
// TS 38.413 §9.4.3 constrains InitiatingMessage/SuccessfulOutcome/
// UnsuccessfulOutcome criticality to the value the elementary procedure object
// declares: NGAP-ELEMENTARY-PROCEDURE.&criticality({NGAP-ELEMENTARY-PROCEDURES}
		...
		for _, file := range pkg.Files {
			ast.Inspect(file, func(n ast.Node) bool {
				...
				if want := ProcedureCriticality(code); crit != want {
					t.Errorf("%s: %s encodes criticality %s, TS 38.413 §9.4.3 fixes it at %s",
