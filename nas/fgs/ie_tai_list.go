...
// TAI is a 5GS tracking area identity: a PLMN and a 3-octet (24-bit) TAC
// (TS 24.501 §9.11.3.8). The wider TAC is a 3GPP-mandated 5GS/EPS difference —
// eps.TAI carries a 2-octet TAC.
type TAI struct {
	PLMN nas.PLMN
	TAC  uint32
}

func (t TAI) String() string { return fmt.Sprintf("%s-%06x", t.PLMN, t.TAC) }

// PartialTAIListType selects how a partial tracking area identity list encodes
// the identities it holds (TS 24.501 §9.11.3.9, table 9.11.3.9.2).
...
func NewTAIList(tais ...TAI) (TAIList, error) {
	if len(tais) == 0 {
		return nil, fmt.Errorf("nas/fgs: TAI list needs at least one identity")
	}

	if len(tais) > maxTAIsTotal {
		return nil, fmt.Errorf("nas/fgs: TAI list holds %d identities, the element carries %d", len(tais), maxTAIsTotal)
	}
...
func ParseTAIList(b []byte) (TAIList, error) {
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("nas/fgs: TAI list is empty")
	}

	if n := out.count(); n > maxTAIsTotal {
		return nil, fmt.Errorf("nas/fgs: TAI list holds %d identities, the element carries %d", n, maxTAIsTotal)
	}

	if len(b) > maxTAIListOctets {
		return nil, fmt.Errorf("nas/fgs: TAI list is %d octets, the element carries %d", len(b), maxTAIListOctets)
	}
...
func parsePartialTAIList(r *nas.Reader) (PartialTAIList, error) {
	...
	// reserved (TS 24.501 table 9.11.3.9.2).
	n := int(hdr&0x1F) + 1
	if n > maxTAIsPerPartialList {
		return PartialTAIList{}, fmt.Errorf("nas/fgs: partial TAI list declares %d identities, want 1-%d", n, maxTAIsPerPartialList)
	}
	...
	case PartialTAIListNonConsecutive, PartialTAIListConsecutive:
		...
		if listType == PartialTAIListConsecutive {
			...
			// A run that would count past the 24-bit TAC is malformed: the
			// identities it claims do not exist.
			if tac+uint32(n)-1 > maxTAC {
				return PartialTAIList{}, fmt.Errorf("nas/fgs: consecutive TAC run from %#x for %d identities exceeds 24 bits", tac, n)
			}

			for i := range n {
				p.TAIs = append(p.TAIs, TAI{PLMN: plmn, TAC: tac + uint32(i)})
			}
	...
	case PartialTAIListPerPLMN:
		...
		return p, nil
	default:
		return PartialTAIList{}, fmt.Errorf("nas/fgs: reserved partial TAI list type %d", listType)
	}
}
...
func (l TAIList) AppendBinary(b []byte) ([]byte, error) {
	if len(l) == 0 {
		return b, fmt.Errorf("nas/fgs: TAI list needs at least one partial list")
	}

	if n := l.count(); n > maxTAIsTotal {
		return b, fmt.Errorf("nas/fgs: TAI list holds %d identities, the element carries %d", n, maxTAIsTotal)
	}
	...
	if n := len(out) - len(b); n > maxTAIListOctets {
		return b, fmt.Errorf("nas/fgs: TAI list encodes to %d octets, the element carries %d", n, maxTAIListOctets)
	}
...
func (p PartialTAIList) write(w *nas.Writer) error {
	if len(p.TAIs) < 1 || len(p.TAIs) > maxTAIsPerPartialList {
		return fmt.Errorf("nas/fgs: partial TAI list holds %d identities, want 1-%d", len(p.TAIs), maxTAIsPerPartialList)
	}

	for _, t := range p.TAIs {
		if t.TAC > maxTAC {
			return fmt.Errorf("nas/fgs: TAC %#x exceeds 24 bits", t.TAC)
		}
	}
	...
	case PartialTAIListPerPLMN:
		...
		return nil
	default:
		return fmt.Errorf("nas/fgs: reserved partial TAI list type %d", p.Type)
	}
}
...
func requireSharedPLMN(tais []TAI) error {
	for _, t := range tais[1:] {
		if t.PLMN != tais[0].PLMN {
			return fmt.Errorf("nas/fgs: partial TAI list type %d needs one PLMN, got %s and %s",
...
func readTAC(r *nas.Reader) (uint32, error) {
	b, err := r.Bytes(3)
	if err != nil {
		return 0, err
	}
...
func writeTAC(w *nas.Writer, tac uint32) {
	w.U8(byte(tac >> 16))
	w.U8(byte(tac >> 8))
	w.U8(byte(tac))
}
