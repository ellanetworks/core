...
//
// It is the shared adversarial-construction primitive for both generations
// (TS 24.007 L3 framing is common to 24.501/5GS and 24.301/EPS); the BuildGMM,
// BuildGSM, BuildEMM and BuildESM constructors seed it with a message header.
//
...
// NewBuilder returns an empty Builder. Prefer the header-seeded constructors
...
func (b *Builder) Bytes() []byte {
	// The Builder only ever issues writes that cannot poison the Writer: it
	// emits every length octet itself so that a declared length may disagree
