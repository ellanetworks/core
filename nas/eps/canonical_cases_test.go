...
var canonicalValues = map[uint8][]byte{
	...
	ieiExtendedProtocolConfigurationOptions: {0x80},
...
func canonicalCases(t *testing.T) []canonicalCase {
	...
	return []canonicalCase{
		...
		},
		{
			...
			order: []canonicalIE{{ieiAPNAMBR, nas.IETLV}, {ieiESMCause, nas.IETV3}, {ieiProtocolConfigurationOptions, nas.IETLV}, {ieiExtendedProtocolConfigurationOptions, nas.IETLVE}},
		},
		{
			name:  "ESMInformationResponse (TS 24.301 §8.3.14)",
			bare:  &ESMInformationResponse{},
			order: []canonicalIE{{ieiAccessPointName, nas.IETLV}, {ieiProtocolConfigurationOptions, nas.IETLV}, {ieiExtendedProtocolConfigurationOptions, nas.IETLVE}},
		},
		{
			name:   "ModifyEPSBearerContextRequest (TS 24.301 §8.3.18)",
			bare:   &ModifyEPSBearerContextRequest{},
			order:  []canonicalIE{{ieiNewEPSQoS, nas.IETLV}, {ieiAPNAMBR, nas.IETLV}, {ieiProtocolConfigurationOptions, nas.IETLV}, {ieiExtendedProtocolConfigurationOptions, nas.IETLVE}},
			values: qos,
		},
		{
			name:  "PDNConnectivityRequest (TS 24.301 §8.3.20)",
			bare:  &PDNConnectivityRequest{},
			order: []canonicalIE{{ieiESMInformationTransferFlag, nas.IETV1}, {ieiAccessPointName, nas.IETLV}, {ieiProtocolConfigurationOptions, nas.IETLV}, {ieiExtendedProtocolConfigurationOptions, nas.IETLVE}},
		},
		{
