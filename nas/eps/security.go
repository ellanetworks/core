...
// nasBearer is the BEARER input to the EPS NAS integrity/cipher algorithms. NAS
// has no radio bearer, so the value is the constant 0 (TS 33.401 §8.1.1).
const nasBearer = nas.BearerEPS

// ErrMACMismatch reports a failed integrity check in Unprotect.
var ErrMACMismatch = fmt.Errorf("nas/eps: %w", nas.ErrMACMismatch)

// ErrSecurityHeaderTypeNotPermitted reports a protected message whose security
// header type the caller did not permit.
var ErrSecurityHeaderTypeNotPermitted = fmt.Errorf("nas/eps: %w", nas.ErrSecurityHeaderTypeNotPermitted)
...
var ErrSequenceNumberMismatch = fmt.Errorf("nas/eps: %w", nas.ErrSequenceNumberMismatch)
...
// keystream (TS 33.401 §6.5). The count's low octet becomes the sequence
// number on the wire.
//
...
func Protect(
	...
	if sht == SHTPlain {
		return nil, fmt.Errorf("nas/eps: %w", nas.ErrNotProtected)
	}

	if !sht.defined() {
		return nil, fmt.Errorf("nas/eps: security header type %s is not defined", sht)
	}

	// The two types this wrapper cannot express: a SERVICE REQUEST is its own
	// frame, and a partially ciphered message enciphers only the containers of a
	// CONTROL PLANE SERVICE REQUEST (TS 24.301 §8.2.25, table 9.3.1).
	if sht == SHTServiceRequest || sht == SHTIntegrityProtectedPartiallyCiphered {
...
// Unprotect verifies a security-protected message and returns the plain EMM
...
// downgrade attempt (TS 24.301 §4.4.4.2, TS 33.401 §7.2.4.4). Passing none accepts
// every type TS 24.301 table 9.3.1 defines for a protected message.
func Unprotect(
	b []byte,
	count nas.Count,
	dir nas.Direction,
	...
	// The sequence number is the count's own low octet (TS 24.301 §4.4.3.1). A
...
// by the (ciphered) NAS message payload (TS 24.301 §4.4.4.1).
