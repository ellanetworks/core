...
// MAC computes the 128-EIA2/128-NIA2 NAS-MAC over msg (TS 33.401 Annex B.2,
// TS 33.501 Annex D.3.1).
func (aesCMACIntegrity) MAC(key IntegrityKey, count uint32, bearer Bearer, direction Direction, msg []byte) ([4]byte, error) {
	m := make([]byte, len(msg)+8)
	binary.BigEndian.PutUint32(m, count)
	m[4] = uint8(bearer)<<3 | uint8(direction)<<2
	copy(m[8:], msg)
...
// Apply enciphers or deciphers data under 128-EEA2/128-NEA2, which are the same
// operation (TS 33.401 Annex B.1, TS 33.501 Annex D.2.1).
func (aesCTRCipher) Apply(key CipherKey, count uint32, bearer Bearer, direction Direction, data []byte) ([]byte, error) {
	var iv [16]byte

	binary.BigEndian.PutUint32(iv[:], count)
	iv[4] = uint8(bearer)<<3 | uint8(direction)<<2
...
func (aesCMACIntegrity) isIntegrity() {}

func (aesCTRCipher) isCipher() {}
